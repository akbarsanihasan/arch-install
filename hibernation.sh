setting_hibernation_grub() {
    ...
    # EXISTING_STUB_OPTIONS=$(grep "options" "{$ROOT_MOUNTPOINT}"/boot/loader/entries/archlinux.conf | sed 's/^options //')
    ...
    # For my hackintosh OpenLinuxBoot
    # sed -i "s|^options.*|${NEW_OPTIONS_STUB}|" "${ROOT_MOUNTPOINT}"/boot/loader/entries/archlinux.conf
